...
class SharedweightsCrossattnAttention(DotProductAttention):
    def forward(
        ...
        if isinstance(split_size, list) and len(split_size) == 4:
            ...
            dots = torch.matmul(qs[0], ks[2].transpose(-1, -2)) * self.scale
            attn = self.softmax(dots)
            attn = self.dropout(attn)
            x1 = torch.matmul(attn, vs[2])  # B H G C
            #x1 = F.scaled_dot_product_attention(qs[0], ks[2], vs[2])
            dots = torch.matmul(qs[1], ks[0].transpose(-1, -2)) * self.scale
            attn = self.softmax(dots)
            attn = self.dropout(attn)
            x2 = torch.matmul(attn, vs[0])  # B H G C
            ...
            if isinstance(split_size, list) and len(split_size) == 2 and split_size[0] == split_size[1]:
                ...
                dots = torch.matmul(qs[0], ks[1].transpose(-1, -2)) * self.scale
                attn = self.softmax(dots)
                attn = self.dropout(attn)
                x1 = torch.matmul(attn, vs[1])  # B H G C
